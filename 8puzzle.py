...
class Node:
    def __init__(self, state, parent, action, depth, name=""):
        self.state = state; self.parent = parent; self.action = action; self.depth = depth; self.name = name
...
def get_neighbors(node):
    ...
    for r, c, act in moves:
        ...
        if 0 <= new_row < 3 and 0 <= new_col < 3:
            ...
            neighbors.append(("".join(state_list), act))
    return neighbors
...
def get_nondeterministic_results(state, intended_action):
    ...
    results = []
    if intended_action in valid_moves: results.append(valid_moves[intended_action])
    slip_map = {"Left": ["Up", "Down"], "Right": ["Up", "Down"], "Up": ["Left", "Right"], "Down": ["Left", "Right"]}
    for slip_act in slip_map.get(intended_action, []):
        if slip_act in valid_moves: results.append(valid_moves[slip_act])
...
def csp_backtracking_generator(goal_state_str, get_name_func, use_fc=False):
    domain_init = ['1', '2', '3', '4', '5', '6', '7', '8', '0']
    domains = [list(domain_init) for _ in range(9)]
    assignment = ['-'] * 9 
    ...
    def recursive_backtrack(var_idx, curr_node):
        if var_idx == 9:
            ...
            yield {"node": curr_node, "domains": domains}, [{"name": curr_node.name, "action": "Chốt", "state": state_str, "is_goal": True, "parent_name": curr_node.parent.name if curr_node.parent else "None", "cost": "Hợp lệ"}], set(), curr_node
            return True
        random.shuffle(domains[var_idx])
        for val in list(domains[var_idx]):
            ...
            if assignment[:var_idx].count(val) > 0:
                yield {"node": child, "domains": domains}, [{"name": child.name, "action": f"Thử {val}", "state": new_state_str, "is_goal": False, "parent_name": curr_node.name, "cost": "Trùng lặp"}], set(), None
                assignment[var_idx] = '-'
                continue
            if val != goal_state_str[var_idx]:
                yield {"node": child, "domains": domains}, [{"name": child.name, "action": f"Thử {val}", "state": new_state_str, "is_goal": False, "parent_name": curr_node.name, "cost": "Sai đích"}], set(), None
                assignment[var_idx] = '-'
                yield {"node": curr_node, "domains": domains}, [{"name": "Undo", "action": f"Gỡ {val}", "state": "".join(assignment), "is_goal": False, "parent_name": child.name, "cost": "Lùi"}], set(), None
                continue
            if use_fc:
                old_domains = [d[:] for d in domains]
                fc_failed = False
                for f_idx in range(var_idx + 1, 9):
                    if val in domains[f_idx]: domains[f_idx].remove(val)
                    if not domains[f_idx]: fc_failed = True
                yield {"node": child, "domains": domains}, [{"name": child.name, "action": f"Gán {val}", "state": new_state_str, "is_goal": False, "parent_name": curr_node.name, "cost": "FC: Cắt domain"}], set(), None
                if fc_failed:
                    yield {"node": child, "domains": domains}, "Cutoff_FC", set(), None
                    for i in range(9): domains[i] = old_domains[i][:]
                    assignment[var_idx] = '-'
                    yield {"node": curr_node, "domains": domains}, [{"name": "Undo", "action": f"Gỡ {val}", "state": "".join(assignment), "is_goal": False, "parent_name": child.name, "cost": "Lùi (FC rỗng)"}], set(), None
                    continue
            else:
                yield {"node": child, "domains": domains}, [{"name": child.name, "action": f"Gán {val}", "state": new_state_str, "is_goal": False, "parent_name": curr_node.name, "cost": "Hợp lệ"}], set(), None
...
def get_eval_value(curr_state, new_state, goal_state, eval_type):
    if eval_type == "Số ô sai vị trí": return sum(1 for s, g in zip(new_state, goal_state) if s != g and s != '0')
    elif eval_type == "Giá trị ô swap":
        if not curr_state: return 9 
        zero_idx_curr = curr_state.find('0')
        return int(new_state[zero_idx_curr])
    else: return get_manhattan_distance(new_state, goal_state)
...
class ScrollableFrame(tk.Frame):
    ...
    def clear(self):
        for widget in self.scrollable_inner_frame.winfo_children(): widget.destroy()
...
def ids_algorithm(start_state, goal_state, get_name_func, reset_name_func=None):
    ...
    while True:
        ...
        if not cutoff_occurred: break
        limit += 1

def simulated_annealing(start_state, goal_state, get_name_func, eval_type, initial_temp=100, cooling_rate=0.95, min_temp=0.1, max_restart=3):
    start_cost = get_manhattan_distance(start_state, goal_state)
    start_node = Node(start_state, None, "Start", 0, get_name_func())
    start_node.cost = start_cost
    if start_state == goal_state:
        data = {"current": start_node, "candidate": None, "best": start_node, "temperature": initial_temp, "accept_prob": 1.0,
                "delta": 0, "accepted": True, "action": "", "cost_history": [start_cost], "temp_history": [initial_temp], "accepted_history": [True],
                "iteration": 0, "initial_temp": initial_temp, "restart_count": 0, "max_restart": max_restart, "min_temp": min_temp}
        yield data, "SA", {start_state}, start_node
        return
    curr = start_node; best = start_node; T = initial_temp
    cost_history = [start_cost]; temp_history = [initial_temp]; accepted_history = [True]
    iteration = 0; restart_count = 0; reached = {start_state}
    while True:
        neighbors = get_neighbors(curr)
        if not neighbors:
            data = {"current": curr, "candidate": None, "best": best, "temperature": T, "accept_prob": 0, "delta": 0, "accepted": False, "action": "",
                    "cost_history": cost_history, "temp_history": temp_history, "accepted_history": accepted_history, "iteration": iteration,
                    "initial_temp": initial_temp, "restart_count": restart_count, "max_restart": max_restart, "min_temp": min_temp}
        ...
        iteration += 1; accepted = False; next_node = None; T *= cooling_rate
        if delta < 0 or random.random() < accept_prob:
            child = Node(new_state, curr, act, curr.depth + 1, get_name_func())
            child.cost = new_cost; next_node = child; reached.add(new_state); accepted = True
            if new_cost < best.cost: best = child
        cost_history.append(new_cost); temp_history.append(T); accepted_history.append(accepted)
        candidate_node = Node(new_state, curr, act, curr.depth + 1, "?")
        candidate_node.cost = new_cost
        is_goal = (new_state == goal_state)
        data = {"current": curr, "candidate": candidate_node, "next": next_node, "best": best, "temperature": T, "accept_prob": accept_prob,
                "delta": delta, "accepted": accepted, "action": act, "cost_history": cost_history, "temp_history": temp_history,
                "accepted_history": accepted_history, "iteration": iteration, "initial_temp": initial_temp, "restart_count": restart_count,
...
def belief_state_bfs(start_states, goal_states, get_name_func, max_depth=15):
    goal_set = frozenset(goal_states); n = len(start_states)
    frontiers = [deque([(s, 0, [])]) for s in start_states]
    explored_list = [{s} for s in start_states]
    solved = [s in goal_set for s in start_states]
    target_goal = None; state_names = {s: get_name_func() for s in start_states}
    all_reached = set(start_states); restart_flag = False; restart_reason = ""
    round_robin_idx = 0
    ...
    while not all(solved):
        found = False
        for _ in range(n):
            i = round_robin_idx % n; round_robin_idx += 1
            if solved[i] or not frontiers[i]: continue
            found = True
            ...
            for act_name in ["Left", "Right", "Up", "Down"]:
                ...
                if new_state not in explored_list[i]:
                    explored_list[i].add(new_state); all_reached.add(new_state)
                    ...
                    frontier_logs.append({"state": new_state, "action": act_name, "from_state": curr_state, "is_goal": is_goal, "depth": depth + 1, "name": state_names[new_state], "belief_idx": i})
                    if is_goal:
                        if target_goal is None:
                            target_goal = new_state; restart_flag = True; restart_reason = f"S{i+1} found goal {new_state}, other states continue toward target..."
                        solved[i] = True
                    if not is_goal and depth + 1 < max_depth: frontiers[i].append((new_state, depth + 1, new_path))
            curr_states = []; queues = []
            for j in range(n):
                if solved[j]: curr_states.append(target_goal if target_goal else start_states[j]); queues.append([])
                elif frontiers[j]: f0 = frontiers[j][0]; curr_states.append(f0[0] if f0 else start_states[j]); queues.append([(s, d, p) for s, d, p in list(frontiers[j])[:6]])
                else: curr_states.append(start_states[j]); queues.append([])
            curr_states = tuple(sorted(curr_states))
            display_data = {"belief": curr_states, "depth": depth + 1, "solved": solved[:], "expanding": i, "expanding_state": curr_state, "frontier_logs": frontier_logs, "explored_sets": explored_list, "all_reached": all_reached, "queues": queues, "target_goal": target_goal, "restart_reason": restart_reason, "restart_flag": restart_flag}
...
class PuzzleApp:
    def __init__(self, root):
        ...
        self.node_counter = 0; self.generator = None; self.auto_id = None
        self.is_auto_running = False; self.is_solved = False
        self.is_sa_mode = False; self.is_belief_mode = False
        self.belief_generated_states = None; self.belief_generated_goals = None
        self.total_popped = 0; self.belief_step_count = 0
        self.setup_ui()
        self.reset_search()

    def get_next_name(self):
        num = self.node_counter; self.node_counter += 1; res = ""; n = num
        while n >= 0: res = chr(65 + (n % 26)) + res; n = n // 26 - 1
        return res
    def reset_name_counter(self): self.node_counter = 0
    ...
    def setup_ui(self):
        ...
        self.algo_cb['values'] = ("BFS Tối ưu (Early Goal + Reached Sớm)", "BFS Cổ điển (Early Goal + Explored Muộn)", "BFS Generic (Late Goal + Reached Muộn)", "DFS (LIFO Stack)", "IDS", "UCS (Uniform Cost Search)", "Greedy Search (Tham lam - Heuristic)", "A* (A Star)", "IDA* (Iterative Deepening A*)", "Leo núi đơn giản (Simple Hill Climbing)", "Leo núi dốc nhất (Steepest-Ascent Hill Climbing)", "Leo núi ngẫu nhiên (Stochastic Hill Climbing)", "Leo núi khởi động lại ngẫu nhiên (Random Restart HC)", "Local Beam Search (Tìm kiếm chùm cục bộ)", "Simulated Annealing (SA - Ủ mô phỏng)", "Belief State BFS (Đa trạng thái)", "AND-OR Graph Search (Không xác định)", "CSP Backtracking (Cơ bản)", "CSP Backtracking (Forward Checking)")
        ...
        tk.Label(self.controls_frame, text="Cost / Heuristic:", bg="white", font=("Arial", 10)).grid(row=1, column=0, sticky="w")
        ...
        self.max_restart_sb.bind("<KeyRelease>", lambda e: self.reset_search()); self.max_restart_sb.bind("<<Increment>>", lambda e: self.reset_search()); self.max_restart_sb.bind("<<Decrement>>", lambda e: self.reset_search())
        ...
        self.k_sb.bind("<KeyRelease>", lambda e: self.reset_search()); self.k_sb.bind("<<Increment>>", lambda e: self.reset_search()); self.k_sb.bind("<<Decrement>>", lambda e: self.reset_search())
    ...
    def draw_state_to_canvas(self, canvas, state_str, domains=None):
        canvas.delete("all")
        ...
        for i, char in enumerate(state_str):
            ...
            if char == '0':
                ...
                if domains and i < len(domains):
                    ds = "".join(domains[i])
                    ds = "\n".join([ds[j:j+3] for j in range(0, len(ds), 3)])
                    canvas.create_text(x0+22, y0+22, text=ds, font=("Arial", 8), fill="#7f8c8d", justify="center")
                else:
    ...
    def create_mini_board_canvas(self, parent, state_str, title="", is_goal=False, domains=None):
        ...
        for i, char in enumerate(state_str):
            ...
            if char == '0':
                ...
                if domains and i < len(domains):
                    canvas.create_text(x0+9, y0+9, text=f"{len(domains[i])}", font=("Arial", 6, "bold"), fill="#7f8c8d")
                else:
    ...
    def update_sa_display(self, data):
        ...
        curr = data.get("current"); candidate = data.get("candidate"); next_node = data.get("next"); best = data.get("best"); accepted = data.get("accepted", False)
        if curr:
        ...
        T = data.get("temperature", 0); initial_temp = data.get("initial_temp", 100)
        self.sa_temp_label.config(text=f"T = {T:.1f}°C"); self.sa_temp_stat.config(text=f"Temp: {T:.1f}°C")
        gauge_w = 380; fraction = min(1.0, T / max(initial_temp, 0.01))
        ...
        self.sa_delta_label.config(text=f"ΔE = {data.get('delta', 0):+d}"); self.sa_prob_label.config(text=f"Accept P = {data.get('accept_prob', 0):.4f}")
        if accepted: self.sa_accepted_label.config(text="Decision: ✅ Accepted", fg="#27ae60")
        else: self.sa_accepted_label.config(text="Decision: ❌ Rejected", fg="#e74c3c")
    ...
    def draw_sa_chart(self, cost_history):
        ...
        cw = max(c.winfo_width() - 40, 600); ch = max(c.winfo_height() - 40, 160); margin = 30
        max_cost = max(cost_history); min_cost = min(cost_history); cost_range = max_cost - min_cost if max_cost != min_cost else 1; n = len(cost_history)
        for i in range(5):
    ...
    def generate_belief_states(self):
        try: n_states = int(self.belief_count_var.get()); n_goals = int(self.belief_goal_count_var.get())
    ...
    def update_belief_display(self, data):
        ...
        states = data.get("belief", ()); solved_list = data.get("solved", []); expanding_idx = data.get("expanding", -1)
        curr_state = data.get("expanding_state", ""); frontier_list = data.get("frontier_logs", [])
        explored_sets = data.get("explored_sets", []); all_reached = data.get("all_reached", set()); queues = data.get("queues", [])
        ...
        solved_count = sum(solved_list) if solved_list else 0; total = len(states); goal_count = len(self.belief_generated_goals or [])
        ...
        self.belief_explored_count.config(text=f"Explored: {total_explored}"); self.belief_front_count.config(text=f"Frontier: {sum(len(q) for q in queues)}")
        restart_reason = data.get("restart_reason", ""); restart_flag = data.get("restart_flag", False); target_goal = data.get("target_goal", None)
        if frontier_list:
            ...
            for item in frontier_list:
                self.add_belief_log_row(item.get("belief_idx", 0), item.get("from_state", curr_state), item.get("action", ""), item.get("state", ""), item.get("is_goal", False), solved_count, total)
        ...
        if queues:
            for qi, q in enumerate(queues):
                ...
                if qi == expanding_idx and curr_state:
                    ...
                    if items_f.winfo_children(): tk.Label(items_f, text="↓", bg="white", fg="#7f8c8d", font=("Arial", 12, "bold")).pack(side=tk.LEFT, padx=2)
                if q:
                    for qs, qd, _ in q:
                        mb = self.create_mini_board_canvas(items_f, qs, title=f"d={qd}"); mb.pack(side=tk.LEFT, padx=1)
                else:
    ...
    def add_log_row(self, curr_node_data, frontier_list, reached_set, domains=None):
        ...
        row_frame.columnconfigure(0, weight=1, minsize=100); row_frame.columnconfigure(1, weight=5, minsize=450); row_frame.columnconfigure(2, weight=3, minsize=300)
        ...
        if isinstance(curr_node_data, list):
            for node in curr_node_data:
                mb = self.create_mini_board_canvas(f_node, node.state, title=f"Node {node.name}\ncost: {node.h}", domains=domains); mb.pack(pady=4)
        elif curr_node_data:
            mb = self.create_mini_board_canvas(f_node, curr_node_data.state, title=f"Node {curr_node_data.name}", domains=domains); mb.pack(pady=4)
        f_front = tk.Frame(row_frame, bg="white")
        f_front.grid(row=0, column=1, sticky="nw", padx=10, pady=5)
        if frontier_list == "Cutoff": tk.Label(f_front, text="CUTOFF (Vi phạm ràng buộc hoặc vượt mốc)", bg="white", fg="#e74c3c", font=("Arial", 11, "bold")).pack(anchor="w")
        elif frontier_list == "Cutoff_FC": tk.Label(f_front, text="CUTOFF: FC phát hiện có ô tương lai bị rỗng Domain!", bg="white", fg="#e74c3c", font=("Arial", 11, "bold")).pack(anchor="w")
        elif frontier_list == "Stuck": tk.Label(f_front, text="KẸT (Đạt cực đại/tiểu cục bộ) ➔ Dừng lại hoặc Restart...", bg="white", fg="#d35400", font=("Arial", 11, "bold")).pack(anchor="w")
        elif not frontier_list: tk.Label(f_front, text="(Không được thêm vào / Thoát vòng lặp)", bg="white", fg="gray", font=("Arial", 10, "italic")).pack(anchor="w")
        else:
            for item in frontier_list:
                ...
                mb = self.create_mini_board_canvas(item_frame, item['state'], domains=domains); mb.pack(side="left", padx=2)
                act_char = item['action'][0] if item['action'] else ""
                info_text = f" , {item['parent_name']} , {act_char} , {item['cost']} }} ➔ Node {item['name']}"
                if "Loại" in str(item['cost']) or "Lùi" in str(item['cost']) or "Trùng" in str(item['cost']) or "Sai" in str(item['cost']): color = "#e74c3c"
                elif item['is_goal'] or "Hợp lệ" in str(item['cost']): color = "#27ae60"
                else: color = "#2c3e50"
    ...
    def add_belief_log_row(self, belief_idx, curr_state, action, new_state, is_goal, solved, total):
        row_frame = tk.Frame(self.log_scroll.scrollable_inner_frame, bg="white", highlightbackground="#ecf0f1", highlightthickness=1)
        row_frame.pack(fill=tk.X, pady=3, padx=5)
        row_frame.columnconfigure(0, weight=1, minsize=80); row_frame.columnconfigure(1, weight=4, minsize=350); row_frame.columnconfigure(2, weight=2, minsize=200)
        ...
        mb = self.create_mini_board_canvas(f_idx, curr_state); mb.pack(pady=2)
        f_front = tk.Frame(row_frame, bg="white")
        ...
        mb2 = self.create_mini_board_canvas(item_frame, new_state); mb2.pack(side=tk.LEFT, padx=2)
    ...
    def add_and_or_log_row(self, data):
        phase = data.get("phase"); depth = data.get("depth", 0)
        row_frame = tk.Frame(self.log_scroll.scrollable_inner_frame, bg="white", highlightbackground="#ecf0f1", highlightthickness=1)
        row_frame.pack(fill=tk.X, pady=3, padx=5)
        if phase == "OR":
            tk.Label(row_frame, text=f"[Độ sâu {depth}] Pha OR - Tại trạng thái:", font=("Arial", 10, "bold"), bg="white", fg="#2980b9").pack(anchor="w", padx=5)
            mb = self.create_mini_board_canvas(row_frame, data["state"]); mb.pack(anchor="w", padx=20, pady=2)
        elif phase == "ACTION":
            ...
            states_frame = tk.Frame(row_frame, bg="white"); states_frame.pack(anchor="w", padx=20, pady=2)
            tk.Label(states_frame, text="↳ Môi trường trượt ra các rủi ro: ", bg="white", font=("Arial", 9)).pack(side=tk.LEFT)
            for s in data["results"]:
                mb = self.create_mini_board_canvas(states_frame, s); mb.pack(side=tk.LEFT, padx=3)
        elif phase == "AND":
    ...
    def show_contingency_plan(self, plan):
        ...
        top = tk.Toplevel(self.root); top.title("Kế hoạch Dự Phòng (Contingency Plan)"); top.geometry("600x500"); top.configure(bg="white")
        ...
        txt.pack(fill=tk.BOTH, expand=True, padx=10, pady=10); txt.insert(tk.END, formatted_plan); txt.config(state=tk.DISABLED)

    def stop_auto(self):
        self.is_auto_running = False
        if self.auto_id: self.root.after_cancel(self.auto_id); self.auto_id = None
        self.btn_auto.config(text="Chạy Tự Động", bg="#2ecc71")

    def toggle_auto(self):
        if self.is_auto_running: self.stop_auto()
        else:
            self.is_auto_running = True; self.btn_auto.config(text="Tạm Dừng", bg="#e74c3c"); self.auto_step()

    def auto_step(self):
        if self.is_auto_running and not self.is_solved:
            self.step_search()
            if self.is_auto_running and not self.is_solved: self.auto_id = self.root.after(350, self.auto_step)

    def reset_search(self):
        self.stop_auto() 
        algo_idx = self.algo_cb.current(); algo = self.algo_var.get(); eval_type = self.eval_var.get()
        is_sa = "Simulated Annealing" in algo; is_belief = "Belief State BFS" in algo
        if self.is_sa_mode and not is_sa: self.destroy_sa_ui(); self.is_sa_mode = False
        if self.is_belief_mode and not is_belief: self.sa_frame.pack_forget(); self.right_title.config(text="MÔ PHỎNG"); self.right_header.pack(fill=tk.X); self.is_belief_mode = False
        if is_sa and not self.is_sa_mode:
            if self.is_belief_mode: self.sa_frame.pack_forget(); self.is_belief_mode = False
            self.right_header.pack_forget(); self.log_scroll.pack_forget(); self.right_title.config(text="MÔ PHỎNG - SIMULATED ANNEALING"); self.is_sa_mode = True; self.sa_frame.pack(fill=tk.BOTH, expand=True); self.build_sa_ui()
        elif is_sa and self.is_sa_mode: self.build_sa_ui()
        if is_belief and not self.is_belief_mode:
            if self.is_sa_mode: self.sa_frame.pack_forget(); self.is_sa_mode = False
            self.right_header.pack_forget(); self.right_title.config(text="MÔ PHỎNG - BELIEF STATE BFS"); self.is_belief_mode = True; self.belief_step_count = 0; self.sa_frame.pack(fill=tk.X); self.log_scroll.pack(fill=tk.BOTH, expand=True); self.log_scroll.clear(); self.build_belief_ui()
        elif is_belief and self.is_belief_mode: self.belief_step_count = 0; self.log_scroll.clear(); self.build_belief_ui()
        ...
        if is_belief:
            ...
            self.start_entry.config(state="disabled"); self.goal_entry.config(state="disabled")
        else:
            for ctrl in belief_ctrls: ctrl.config(state="disabled")
            self.start_entry.config(state="normal"); self.goal_entry.config(state="normal")
        start_state = self.start_entry.get().strip(); goal_state = self.goal_entry.get().strip()
        if not is_belief and "CSP" not in algo:
        ...
        if is_sa:
            ...
            try:
                initial_temp = float(self.sa_initial_temp_var.get()); cooling_rate = float(self.sa_cooling_rate_var.get()); min_temp = float(self.sa_min_temp_var.get()); max_restart_sa = int(self.sa_max_restart_var.get())
            except ValueError: initial_temp, cooling_rate, min_temp, max_restart_sa = 100, 0.95, 0.1, 3
        else:
        ...
        elif "CSP Backtracking (Cơ bản)" in algo: self.generator = csp_backtracking_generator(goal_state, self.get_next_name, use_fc=False)
        elif "CSP Backtracking (Forward Checking)" in algo: self.generator = csp_backtracking_generator(goal_state, self.get_next_name, use_fc=True)
        elif is_belief:
            if self.belief_generated_states and self.belief_generated_goals: self.generator = belief_state_bfs(self.belief_generated_states, self.belief_generated_goals, self.get_next_name)
            else: self.generator = None; messagebox.showinfo("Belief State", "Hãy nhấn 'Sinh ngẫu nhiên' trước!")
        else: self.generator = dfs_algorithm(start_state, goal_state, self.get_next_name)
        self.btn_step.config(state="normal" if self.generator else "disabled"); self.btn_auto.config(state="normal" if self.generator else "disabled")
        ...
        try:
            ...
            if self.is_sa_mode:
                ...
                if frontier_logs == "SA_stuck":
                    self.lbl_status.config(text="❌ SA kết thúc: Đạt T_min hoặc không tìm thấy lời giải!", fg="#c0392b"); self.is_solved = True; self.btn_step.config(state="disabled"); self.stop_auto(); self.btn_auto.config(state="disabled")
                elif goal_node:
                    self.is_solved = True; self.lbl_status.config(text=f"🎉 SA tìm thấy Đích! Best cost: {goal_node.cost}", fg="#27ae60"); self.btn_step.config(state="disabled"); self.stop_auto(); self.btn_auto.config(state="disabled")
                else: self.lbl_status.config(text=f"Trạng thái: SA đang chạy (Iter {curr_data.get('iteration', 0)})", fg="#2980b9")
                return
            if self.is_belief_mode:
                if isinstance(frontier_logs, str) and frontier_logs == "Belief_limit":
                    self.is_solved = True; self.lbl_status.config(text="❌ Belief BFS: Đạt giới hạn độ sâu!", fg="#c0392b"); self.btn_step.config(state="disabled"); self.stop_auto(); self.btn_auto.config(state="disabled")
                    return
                ...
                if goal_node:
                    self.is_solved = True; solved_count = sum(curr_data.get('solved', [])); total = len(curr_data.get('solved', []))
                    self.lbl_status.config(text=f"🎉 Belief BFS: {solved_count}/{total} states đã tìm thấy goal!", fg="#27ae60"); self.btn_step.config(state="disabled"); self.stop_auto(); self.btn_auto.config(state="disabled")
                else:
                    solved_count = sum(curr_data.get('solved', [])); total = len(curr_data.get('solved', []))
            ...
            if frontier_logs == "AND-OR":
                ...
                if phase == "OR": self.draw_state_to_canvas(self.curr_board_canvas, curr_data["state"]); self.lbl_status.config(text=f"Trạng thái: Đang xét Pha OR (Tìm hành động)")
                elif phase == "ACTION": self.lbl_status.config(text=f"Trạng thái: Thử hành động {curr_data['action']} (Phát sinh {len(curr_data['results'])} trạng thái)")
                elif phase == "AND": self.lbl_status.config(text=f"Trạng thái: Đang xét Pha AND (Duyệt các nhánh rủi ro)")
                self.lbl_depth.config(text=f"Độ sâu đệ quy: {curr_data.get('depth', 0)}"); self.lbl_popped.config(text=f"Số bước mô phỏng: {self.total_popped}")
                return
            domains = None
            ...
            if best_node:
                ...
                self.lbl_manhattan.config(text=f"Chi phí / Heuristic tới Đích: {md}"); self.lbl_depth.config(text=f"Độ sâu hiện tại (Depth): {best_node.depth}")
            self.lbl_popped.config(text=f"Số Bước đã duyệt: {self.total_popped}")
            if goal_node:
                self.is_solved = True
                if "CSP" in self.algo_var.get(): self.lbl_status.config(text=f"🎉 CSP đã tạo thành công bàn cờ hợp lệ y hệt Goal!", fg="#27ae60")
                else: path = self.trace_path(goal_node); self.lbl_status.config(text=f"🎉 Đã tìm thấy Đích!\nCost: {goal_node.depth} bước\nĐường đi: {path}", fg="#27ae60")
            ...
            if "AND-OR" in self.algo_var.get():
                plan = e.value; self.is_solved = True; self.stop_auto(); self.btn_step.config(state="disabled"); self.btn_auto.config(state="disabled")
                if plan == "failure" or plan is None: self.lbl_status.config(text="❌ Thất bại: Không tìm được Contingency Plan an toàn!", fg="#c0392b")
                else: self.lbl_status.config(text="🎉 Đã tìm thấy Contingency Plan! (Mở cửa sổ mới)", fg="#27ae60"); self.show_contingency_plan(plan)
                return
            if "CSP" in self.algo_var.get(): self.lbl_status.config(text="❌ CSP Thất bại: Cạn kiệt không gian mẫu / Không thể tìm thấy Goal!", fg="#c0392b")
            elif self.is_sa_mode: self.lbl_status.config(text="❌ SA kết thúc: Không thể tiếp tục!", fg="#c0392b")
            elif self.is_belief_mode: self.lbl_status.config(text="❌ Belief BFS: Không tìm thấy lời giải!", fg="#c0392b")
            else: self.lbl_status.config(text="❌ Thất bại: Đạt cực đại cục bộ / Đã duyệt hết!", fg="#c0392b")
            self.is_solved = True; self.btn_step.config(state="disabled"); self.stop_auto(); self.btn_auto.config(state="disabled")
                
    def trace_path(self, node):
        path = []; curr = node
        while curr:
            if curr.action and not curr.action.startswith("Start") and curr.action != "Start": path.append(f"{curr.action[0]}")
            curr = curr.parent
        return " -> ".join(path[::-1])
